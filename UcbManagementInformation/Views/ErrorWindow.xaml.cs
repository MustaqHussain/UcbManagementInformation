namespace UcbManagementInformation
{
    ...
    public partial class ErrorWindow : ChildWindow
    {
        ...
        protected ErrorWindow(string message, Exception errorDetails, Guid handlingInstanceId)
        {
            InitializeComponent();
            MessageText.Text = message;
            ErrorTextBox.Text = CreateExceptionDetails(errorDetails);
            HandlingGuidText.Text = handlingInstanceId.ToString();
            ...
            {
                HandlingGuidText.Visibility = Visibility.Collapsed;
                HandlingGuidLabel.Visibility = Visibility.Collapsed;
            }
        }
        ...
        /// <param name="redirectToErrorPageOnClose">Flag that specifies if the application should redirect to the error page when closed.</param>
        public static void CreateNew(string message, Guid handlingInstanceID, bool redirectToErrorPageOnClose)
        {
            CreateNew(message, null, handlingInstanceID, redirectToErrorPageOnClose);
        }
        ...
        /// <param name="handlingInstanceID">The handling instance ID</param>
        /// <param name="redirectToErrorPageOnClose">Flag that specifies if the application should redirect to the error page when closed.</param>
        public static void CreateNew(Exception exception, Guid handlingInstanceID, bool redirectToErrorPageOnClose)
        {
            CreateNew(exception.Message, exception, handlingInstanceID, redirectToErrorPageOnClose);
        }
        ...
        /// <param name="handlingInstanceID">The handling instance ID</param>
        /// <param name="redirectToErrorPageOnClose">Flag that specifies if the application should redirect to the error page when closed.</param>
        public static ErrorWindow CreateNew(string message, Exception exception, Guid handlingInstanceID, bool redirectToErrorPageOnClose)
        {
            ErrorWindow window = new ErrorWindow(message, exception, handlingInstanceID);

            if (redirectToErrorPageOnClose)
            {
                window.Closed += delegate { RedirectFromErrorWindow(); };
            }
        ...
        private static void RedirectFromErrorWindow()
        {
            HtmlPage.Window.Navigate(new Uri("UcbManagementInformationHost.aspx", UriKind.Relative));
        }
        ...
        private void Show_Click(object sender, RoutedEventArgs e)
        {
            ...
            this.LabelText.Visibility = Visibility.Visible;
        }

        private void CopyButton_Click(object sender, RoutedEventArgs e)
        {
            Clipboard.SetText("Handling ID = " + HandlingGuidText.Text + "\r\n" + this.ErrorTextBox.Text);
            MessageBox.Show("The error details have succesfully been copied to the clipboard.");
        }
